...
var (
	encodeMap = []byte("0123456789ABCDEFGHJKMNPQRSTVWXYZ")
	decodeMap = [256]byte{
...
func EncodeWithPadding(dst, src []byte, padChar rune) {
	for len(src) > 0 {
		...
		if size >= 8 {
			...
			dst[0] = encodeMap[b[0]&31]
			dst[1] = encodeMap[b[1]&31]
			dst[2] = encodeMap[b[2]&31]
			dst[3] = encodeMap[b[3]&31]
			dst[4] = encodeMap[b[4]&31]
			dst[5] = encodeMap[b[5]&31]
			dst[6] = encodeMap[b[6]&31]
			dst[7] = encodeMap[b[7]&31]
		} else {
			for i := 0; i < size; i++ {
				dst[i] = encodeMap[b[i]&31]
			}
		}
